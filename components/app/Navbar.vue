...
<template>
  <Disclosure as="nav" class="fixed top-0 lg:top-4 sm:top-0 left-0 right-0 mx-auto max-w-7xl md:bg-white md:dark:bg-[#281c1f] bg-transparent md:border md:border-gray-200 md:dark:border-gray-200/10 md:rounded-lg md:shadow-lg z-50"
    v-slot="{ open }">
          ...
          <div class="hidden sm:ml-6 sm:flex sm:space-x-8">
            <template v-for="navItem in routesRef">
              <NuxtLink v-if="navItem.target !== '_blank'" :to="navItem.to"
                ...
                :class="navItem.active ? 'border-[#B4878E] text-gray-900 dark:text-gray-50' : 'border-transparent hover:border-gray-300 dark:hover:border-gray-500 hover:text-gray-700 text-gray-500 dark:text-gray-200'">
                {{ navItem.name }}
              </NuxtLink>
        ...
        <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
          <div class="flex gap-4 items-center md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
            ...
            <NuxtLink :to="config?.actionButton?.href"
              class="hidden md:block text-white bg-[#B4878E] hover:bg-[#986670] dark:bg-[#986670] focus:ring-4 focus:outline-none  font-medium rounded-lg text-sm px-4 py-2 text-center dark:hover:bg-[#B4878E]">
    ...
    <DisclosurePanel class="sm:hidden">
      <div class="space-y-1 pb-4 pt-2">
        <DisclosureButton v-for="item in routesRef" as="a" :href="item.to"
          ...
          :class="item.active ? 'bg-primary-50 border-[#B4878E] text-primary-700' : 'border-transparent text-gray-500 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-700'">
          {{ item.name }}
      ...
      <div class="border-t border-gray-200 pb-3 pt-4">
        <div class="flex items-center px-4">
          <NuxtLink :to="config?.actionButton?.href"
            class="text-white bg-[#B4878E] hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 text-center ">
            {{ config?.actionButton?.text }}
          </NuxtLink>
